...
open(8,file='/Volumes/nozoe/S/S_'//datenow//'.txt',status='replace')
write(8,'(A3,1X,A3,1X,A3,1X,A4)')'lon','lat','dep','sal'
do v=1,l
     do j=1,lj
          do i=1,li
               if (abs(S(i,j,k)).LE.45.)then
                write(8,'(F11.2,1X,F11.2,1X,F5.1,1X,F11.6)')lon(i),lat(j),depth(v),S(i,j,dev(v))
               else
