! gfortran U_read_0316.f90 -L/usr/local/Cellar/netcdf/4.8.1/lib -lnetcdff
program main
...
U(1:720,:,1)=Ut(3601:4320,241:1201,18,1)*scale_factor1+add_offset1
U(721:1081,:,1)=Ut(1:361,241:1201,18,1)*scale_factor1+add_offset1
U(1:720,:,2)=Ut(3601:4320,241:1201,25,1)*scale_factor1+add_offset1
U(721:1081,:,2)=Ut(1:361,241:1201,25,1)*scale_factor1+add_offset1
open(7,file='/Volumes/south_pacific/U/U_'//datenow//'.dat')
do k=1,ll
     do j=1,lj
          do i=1,li
               if (abs(U(i, j,k)).GE.5.) U(i,j,k)=0.
          end do 
     end do
end do
write(7,*)(((U(i,j,k),i=1,li),j=1,lj),k=1,ll)
close(7)
