! use gfortran 
! gfortran read_grid0318.f90 -L/usr/local/Cellar/netcdf/4.8.1/lib -lnetcdff
!一起读会超容量，上下两部分要分开读
program read_grid
implicit none
...
! 	   stop
! 	end if
! HH(:,:,1)=HHt(3361:4000,800:1321,18,1)
! HH(:,:,1)=HHt(3361:4000,800:1321,18,1)
! HH(:,:,2)=HHt(3361:4000,800:1321,25,1)
! HH(:,:,2)=HHt(3361:4000,800:1321,25,1)
...
! open(7,file='/Volumes/south_pacific/set_input/grid_coordinates.dat')
! write(7,*)(DD(k), k = 1, ll)
...
H(1:720,:)=Ht(3601:4320,241:1201,1)
H(721:1081,:)=Ht(1:361,241:1201,1)
...
LEV(1:720,:)=LEVt(3601:4320,241:1201,1)
LEV(721:1081,:)=LEVt(1:361,241:1201,1)
...
open(8,file='/Volumes/south_pacific/set_input/grid_mask_bathy.dat',status='replace')
