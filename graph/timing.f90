program count
	...
	integer::i,j,ii,jj,III,JJJ,YS,N,P,IP,YI,NKAI,land,area,area_number(14),area_number_day(14,NDAY+1),nc,a,y,INTY
		...
		if((III<=li).and.(JJJ<=lj))then
			! if(seashore(iii,jjj)>0.5)then!第一层
				...
				if(switch(area,ip)==0)then!防止重复计算的switch
					...
					ii=INT((RX(IP)-X_edge)*12)+1
					jj=INT((RY(IP)-Y_edge)*12)+1
					num_spawn(ii,jj)=num_spawn(ii,jj)+1
