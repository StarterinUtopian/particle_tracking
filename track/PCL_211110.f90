! gfortran PCL_0921.f90 -L/usr/local/Cellar/netcdf/4.8.0_2/lib -lnetcdff
module globals
     implicit none
     save 
     ...
     integer,parameter::NSTART=36,NDAY=250,DDT=1200,dall=4924!計算
     real,parameter::PDIFH=100!扩散系数

     type d3
     real,dimension(NPCL):: X,Y,Z!格子宽!粒子的经纬度!离原点距离
     end type

     type d2
     real,dimension(NPCL)::X,Y
     end type

     type physics
     real,dimension(li,lj,ll)::U,V,S
     end type
     ...
     subroutine settings(H,MASK,IDSX,PCLX,PCLY,STARTDATE,dateall)
          ...
          character,intent(out),dimension(dall)::dateall*8
     ...
     subroutine readin(dateyear,U,V,S)
          ...
          U(:,:,:)=Ut(3361:4000,800:1321,1:25,1)*scale_factor+add_offset
     ...
     subroutine boundary(H,MASK,PCLX,PCLY,PCLZ,IDSX,LLL)! 判断是否触岸和触底
          real,intent(inout)::PCLX,PCLY,PCLZ
...
program pcl_track
     ...
     integer:: i,j,k,LLL
     integer::YI,YN,YS,ip
     integer,dimension(ip)::land!着陆开关
     !CURRENT VELOVITY
     type(physics)::physic0,physic1
     ...
     type(d2)::PS,PC,PD
     integer:: NKAI
     character:: startdate(NSTART)*8,dateyear(NDAY+1)*8,dateall(dall)*8
     ! 读所有初始日期


     ! 读格子,初始位置，开始时间
     call settings(H,MASK,IDSX,PCLX,PCLY,STARTDATE,dateall)
     do YS=1,NSTART
          ...
          do NKAI = 0, MAXKAI
           ...
           do IP=1,NPCL      
                hour=INT((MOD(INT(NKAI*DDT),24*3600)/3600))
                    ...
                    PCLX=PCLX0+PCLXC+PCLXD+PCLXS
                    PCLY=PCLY0+PCLYC+PCLYD+PCLYS
                    call boundary(H,MASK,PCLX(IP),PCLY(IP),IDSX(IP),LLL)
                    IDSX0=IDSX(IP)
