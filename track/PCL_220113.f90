...
subroutine dep_lev(PCLX,PCLY,IDSX,NKAI,H,deplev,LLL)!PCLX(IP),PCLY(IP),IDSX(IP),NKAI,H,deplev,LLL
     ...
     subroutine boundary(H,MASK,LLL,IDSX,PCLX1,PCLY1,PCLX,PCLY,land) !判断是否触岸和触底
         ! call boundary(H,MASK,deplev,PCLX0,PCLY0,PCLX1,PCLY1,PCLX(IP),PCLY(IP),IDSX0,IDSX(IP))

          real,intent(in)::H(li,lj)
          integer,intent(in)::MASK(li,lj,ll),LLL
          real,intent(in)::IDSX
          ...
          if((III<=1).OR.(III>=li))LAND=1
          if ((JJJ<=1).OR.(JJJ>=lj))LAND=1
          if(land<0.5)then
               PCLX=PCLX1
               PCLY=PCLY1
          endif 
     ...
     subroutine output(IP,NKAI,YI,PCLX,PCLY,RPCLX,RPCLY,IDSX,land)
          ...
          IDSX= (111.111*1000*cos(RPCLY/360.*2*pi))/12.
           write(*,'(I6,1X,I6,1X,I6,1X,F11.6,1x,F11.6,1X,I6)')&
          NKAI,IP,YI,RPCLX,RPCLY,land
          write(14,'(I6,1X,I6,1X,I6,1X,F11.6,1x,F11.6,1X,I6)')&
          NKAI,IP,YI,RPCLX,RPCLY,land


          if ((land>0.5).and.(yes(IP)==0))then
          write(15,'(I6,1X,I6,1X,F11.6,1x,F11.6)')&
          NKAI,IP,RPCLX,RPCLY
          yes(IP)=1
           end if
...
program pcl_track
     ...
     ! 读格子,初始位置，开始时间
          ...
          NSSS=NDAY*24*3600        
          MAXKAI=INT(NSSS/DDT)
          call settings(DD,HH,H,MASK,lev,IDSX,PCLX,PCLY,STARTDATE)!H,MASK,deplev,IDSX,PCLX,PCLY,STARTDATE
          ...
          do YS=1,NSTART  
           ...
           do IP=1,NPCL  
               if(land(IP)<0.5)then !着陆开关   
                    ...
                    call boundary(H,MASK,LLL,IDSX(IP),PCLX1,PCLY1,PCLX(IP),PCLY(IP),land(IP))
                     if(MOD(NKAI*DDT, 24*3600) == 0)then
