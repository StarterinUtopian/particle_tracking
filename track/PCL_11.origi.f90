! gfortran PCL_11.origi.f90 -L/usr/local/Cellar/netcdf/4.8.0_2/lib -lnetcdff
module globals
     implicit none
    ...
    open(3,file='/Volumes/erika/CMEMS/set_input/start-timing/dateall.txt')
     ...
     subroutine settings(H,MASK,deplev,IDSX,PCLX,PCLY,STARTDATE)

          real,intent(out),dimension(li,lj)::H
          integer,intent(out),dimension(li,lj)::deplev
     ...
     ! 读格子
          ...
          read(7,*)((deplev(i, j), i = 1, li), j = 1, lj)
          close(7)
...
subroutine dep_lev(PCLX,PCLY,PCLZ,IDSX,NKAI,H,deplev,LLL,DD)!PCLX(IP),PCLY(IP),IDSX(IP),NKAI,H,deplev,LLL
     real,dimension(li,lj),intent(in)::H
     integer,dimension(li,lj),intent(in)::deplev
     real,dimension(ll),intent(in)::DD
     real,intent(in)::PCLX,PCLY,PCLZ,IDSX
...
　　　　　　end if  
     ...
     subroutine boundary(H,MASK,deplev,IDSX,PCLX1,PCLY1,PCLX,PCLY,land) !判断是否触岸和触底
         ! call boundary(H,MASK,deplev,PCLX0,PCLY0,PCLX1,PCLY1,PCLX(IP),PCLY(IP),IDSX0,IDSX(IP))
          ...
          if((deplev(III,JJJ)<=1.0).or.(H(III,JJJ)<1.0))then
               land=1
          end if
           if((III>=1).and.(JJJ>=1))then
     ...
     subroutine output(IP,NKAI,YI,PCLX,PCLY,RPCLX,RPCLY,IDSX,land)
          ...
          if (land>0.5)then
          write(15,'(I6,1X,I6,1X,I6,1X,F11.6,1x,F11.6)')&
          NKAI,IP,YI,RPCLX,RPCLY
           end if
...
program pcl_track
     ...
     integer,dimension(li,lj)::deplev
     ...
     ! 读格子,初始位置，开始时间
          ...
          NSSS=NDAY*24*3600        
          MAXKAI=INT(NSSS/DDT)
          call settings(H,MASK,deplev,IDSX,PCLX,PCLY,STARTDATE)!H,MASK,deplev,IDSX,PCLX,PCLY,STARTDATE
 

          do YS=1,NSTART  
               call count_date(startdate(YS),dateyear) 
               open(14,file='/Volumes/erika/CMEMS/output/all/'//startdate(YS)//'.dat')
               open(15,file='/Volumes/erika/CMEMS/output/land/'//startdate(YS)//'.dat')
           ...
           do IP=1,NPCL  
               if(land(IP)<0.5)then !着陆开关   
                    call dep_lev(PCLX(IP),PCLY(IP),IDSX(IP),NKAI,H,deplev,LLL)
                    call adverage(U0,V0,S0,PCLX(IP),PCLY(IP),IDSX(IP),NKAI,LLL,PCLU0,PCLV0,SX0,SY0)
                    ...
                    call boundary(H,MASK,deplev,IDSX(IP),PCLX1,PCLY1,PCLX(IP),PCLY(IP),land(IP))
