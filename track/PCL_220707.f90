...
module globals
     ...
     integer,parameter::NPCL=900  !投入粒子数
     ...
     contains
          subroutine count_date(startdate,dateyear)!時間換算用
    ...
    open(3,file='/Volumes/erika/CMEMS/set_input/start-timing/dateall.txt')
     ...
     subroutine settings(DD,HH,H,MASK,lev,IDSX,PCLX,PCLY,STARTDATE)!格子点データ入力、時間設定
     ...
     ! 读初始坐标转换成距离
     open(8, file = '/Volumes/erika/CMEMS/set_input/start-point/release_sixpoints.dat')
     ...
     ! end subroutine readin
          ...
          subroutine readin(dateyear,U,V,S)!流速入力
...
subroutine dep_lev(PCLX,PCLY,IDSX,NKAI,H,deplev,LLL)!鉛直移動水層設定
     ...
     subroutine adverage(U,V,S,PCLX,PCLY,IDSX,NKAI,LLL,PCLU,PCLV,SX,SY)!流速平均
     ...
     subroutine dispersion(PCLXD,PCLYD)!拡散
     ...
     subroutine boundary(H,MASK,LEV,LLL,IDSX,PCLX1,PCLY1,PCLX,PCLY,land) !着岸と着底判断
     ...
     subroutine output(IP,NKAI,YI,PCLX,PCLY,RPCLX,RPCLY,IDSX,land)!出力
...
program pcl_track
     ...
     ! 读格子,初始位置，开始时间
          ...
          do YS=1,1
               ...
               open(14,file='/Volumes/erika/CMEMS/output/all/sixpoints_'//startdate(YS)//'.dat',status='replace')
